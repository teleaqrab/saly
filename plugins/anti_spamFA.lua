  ...
  if msg.from.type == 'user' then
    ...
    if msgs > max_msg then
	  ...
	  if msg.to.type == 'user' then
		...
		if msgs >= max_msg then
			print("Pass2")
			send_large_msg("user#id"..msg.from.id, "کاربر ["..msg.from.id.."] بلاک شد به دلیل اسپم زدن.")
	  ...
	  if msg.to.type == 'chat' or msg.to.type == 'channel' then
		if username then
			...
			send_large_msg(receiver , "ارسال تند و سریع اینجا ممنوع اس\n@"..username.."["..msg.from.id.."]\nوضعیت:کاربر پاک شد ")
		else
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] kicked for #spam")
			send_large_msg(receiver , "ارسال تند و سریع اینجا ممنوع است\nنام:"..name_log.."["..msg.from.id.."]\nوضعیت: کاربر پاک شد")
		end
	  end
      -- incr it on redis
      ...
      if gbanspamonredis then
        if tonumber(gbanspamonredis) ==  4 and not is_owner(msg) then
          ...
          send_large_msg("chat#id"..msg.to.id, "کاربر [ "..name.." ]"..msg.from.id.." گلوبال بند شد (به دلیل اسپم زدن)")
		  send_large_msg("channel#id"..msg.to.id, "کاربر [ "..name.." ]"..msg.from.id.." گلوبال بند شد (به دلیل اسپم زدن)")
          local GBan_log = 'GBan_log'
		  local GBan_log =  data[tostring(GBan_log)]
		  for k,v in pairs(GBan_log) do
			log_SuperGroup = v
			gban_text = "کاربر [ "..name.." ] ( @"..username.." )"..msg.from.id.." گلوبال بند شد ( "..msg.to.print_name.." ) [ "..msg.to.id.." ] (به دلیل اسپم زدن)"
