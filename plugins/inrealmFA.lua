...
local function create_realm(msg)
	...
	if is_sudo(msg) or is_realm(msg) and is_admin1(msg) then
		local group_creator = msg.from.print_name
		create_group_chat (group_creator, group_name, ok_cb, false)
		return 'گروه مدیران [ '..string.gsub(group_name, '_', ' ')..' ] ساخته شد.'
	end
end
...
local function lock_group_member(msg, data, target)
    ...
    local group_member_lock = data[tostring(target)]['settings']['lock_member']
        if group_member_lock == 'yes' then
            return 'اعضای گروه همچنان قفل است.'
        else
...
local function unlock_group_member(msg, data, target)
    ...
    local group_member_lock = data[tostring(target)]['settings']['lock_member']
        if group_member_lock == 'no' then
            return 'ممبرهای گروه قفل نیستند'
        else
            data[tostring(target)]['settings']['lock_member'] = 'no'
            save_data(_config.moderation.data, data)
        return 'ممبرهای گروه باز شدند'
	end
end
...
local function lock_group_photo(msg, data, target)
    ...
    local group_photo_lock = data[tostring(target)]['settings']['lock_photo']
        if group_photo_lock == 'yes' then
            return 'عکس گروه همچنان قفل است.'
        else
            data[tostring(target)]['settings']['set_photo'] = 'waiting'
            save_data(_config.moderation.data, data)
        end
	return 'لطفا عکس گروه را ارسال کنید.'
end
...
local function realms_list(msg)
	...
	if not data[tostring(realms)] then
		return 'ریلایمی درحال حاظرموجود نمیباشد.'
	end
	local message = 'لیست ریلایم:\n'
	for k,v in pairs(data[tostring(realms)]) do
		...
		local group_owner = "اونری نیست."
		if data[tostring(v)]['admins_in'] then
			group_owner = tostring(data[tostring(v)]['admins_in'])
		end
		local group_link = "لینکی نیست."
		if data[tostring(v)]['settings']['set_link'] then
			group_link = data[tostring(v)]['settings']['set_link']
		end
		message = message .. '- '.. name .. ' (' .. v .. ') ['..group_owner..'] \n {'..group_link.."}\n"
...
local function admin_user_promote(receiver, member_username, member_id)
        ...
        if data['admins'][tostring(member_id)] then
            return send_large_msg(receiver, '@'..member_username..' ادمین ربات است.')
        end
        data['admins'][tostring(member_id)] = member_username
        save_data(_config.moderation.data, data)
	return send_large_msg(receiver, '@'..member_username..' به عنوان ادمین ربات انتخاب شد.')
end
...
local function admin_user_demote(receiver, member_username, member_id)
	...
	if not data['admins'][tostring(member_id)] then
		send_large_msg(receiver, "@"..member_username..' ادمین ربات نیست.')
		return
    end
	data['admins'][tostring(member_id)] = nil
	save_data(_config.moderation.data, data)
	send_large_msg(receiver, 'ادمین ربات @'..member_username..' .از ادمینی خلع شد.')
end
...
local function username_id(cb_extra, success, result)
   ...
   local text = 'کاربری با ایدی @'..member..' دراین گروه نیست.'
   for k,v in pairs(result.members) do
...
local function res_user_support(cb_extra, success, result)
   ...
   local support_id = result.peer_id
	if get_cmd == 'addsupport' then
		...
		send_large_msg(receiver, "کاربر ["..support_id.."] به عنوان پشتیبان تیم انتخاب شد.")
	elseif get_cmd == 'removesupport' then
		support_remove(support_id)
		send_large_msg(receiver, "کاربر ["..support_id.."] از پشتیبانی تیم خلع شد.")
	end
end
...
function run(msg, matches)
    ...
    if matches[1] == 'گروه' and matches[2] then
	...
	--[[ Experimental
	if matches[1] == 'سوپرگروه' and matches[2] then
    ...
    if matches[1] == 'ریلایم' and matches[2] then
			if not is_sudo(msg) then
    ...
    local receiver = get_receiver(msg)
			...
			if matches[1] == 'setabout' and matches[2] == 'sgroup'and is_realm(msg) then
				...
				save_data(_config.moderation.data, data)
				return "شرح گروه ست شد برای : ["..matches[2]..']'
			end
		...
		if matches[1] == 'addadmin' then
			...
			if string.match(matches[2], '^%d+$') then
				...
				print("user "..admin_id.." ادمین اصلی ربات شد.")
		...
		if matches[1] == 'removeadmin' then
			...
			if string.match(matches[2], '^%d+$') then
				local admin_id = matches[2]
				print("user "..admin_id.." ازکمک مدیری خلع شد.")
		...
		if matches[1] == 'support' and matches[2] then
			if string.match(matches[2], '^%d+$') then
				...
				print("کاربر "..support_id.." به پشتیبان تیم اضافه شد")
				support_add(support_id)
				return "کاربر ["..support_id.."]به پشتیبان تیم اضافه شد"
			else
				local member = string.gsub(matches[2], "@", "")
				local receiver = get_receiver(msg)
		...
		if matches[1] == '-support' then
			if string.match(matches[2], '^%d+$') then
				local support_id = matches[2]
				print("کاربر "..support_id.." ازپشتیبانی ربات خلع شد.")
				support_remove(support_id)
				return "User ["..support_id.."] ازپشتیبانی ربات خلع شد."
			else
				local member = string.gsub(matches[2], "@", "")
				local receiver = get_receiver(msg)
		...
		if matches[1] == 'list' and matches[2] == 'realms' then
			...
			elseif msg.to.type == 'user' then
				...
				return "لیست ریلایم ساخته شد." --realms_list(msg)
			end
		end
