local function set_bot_photo(msg, success, result)
  ...
  if success then
    ...
    send_large_msg(receiver, 'تصویر عوض شد', ok_cb, false)
    redis:del("bot:photo")
  else
...
local function logrem(msg)
	...
	save_data(_config.moderation.data, data)
	local text = 'لوگ سوپرگروه پاک شد!'
	reply_msg(msg.id,text,ok_cb,false)
	return
end
...
local function run(msg,matches)
    ...
    if matches[1] == "setbotphoto" then
    	...
    	return 'لطفا عکس بوت را ارسال کنید.'
    end
    if matches[1] == "markread" then
    	if matches[2] == "on" then
    		redis:set("bot:markread", "on")
    		return "علامت گذاری به عنوان خانده شده > on"
    	end
    	if matches[2] == "off" then
    		redis:del("bot:markread")
    		return "علامت گذاری به عنوان خانده شده> off"
    	end
    	return
    end
    ...
    if matches[1] == "contactlist" then
      ...
      return "من ارسال میکنم کانتکت هارابه صورت فایل در پی وی."
    end
    if matches[1] == "delcontact" then
      ...
      return "یوزر"..matches[2].." از کانتکت لیست پاک شد."
    end
   ...
   return "یوزر به وسیله شماره تلفن +"..matches[2].." اد شد."
end
 ...
 if matches[1] == "mycontact" and is_sudo(msg) then
	if not msg.from.phone then
		return "من باید شماره شمارا داشته باشم."
    end
