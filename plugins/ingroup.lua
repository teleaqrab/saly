...
local function check_member_realm_add(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      data[tostring(realms)][tostring(msg.to.id)] = msg.to.id
      save_data(_config.moderation.data, data)
      return send_large_msg(receiver, 'گروه به عنوان گروه مدیران اضافه شد!')
    end
  end
end
...
local function check_member_modadd(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      data[tostring(groups)][tostring(msg.to.id)] = msg.to.id
      save_data(_config.moderation.data, data)
      return send_large_msg(receiver, 'گروه اضافه شد و شما به عنوان ادمین وصاحب آن انتخاب شدید. ')
    end
  end
end
...
local function check_member_realmrem(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
      return send_large_msg(receiver, 'گروه مدیران حذف شد!')
    end
  end
end
...
local function set_descriptionmod(msg, data, target, about)
  ...
  save_data(_config.moderation.data, data)
  return 'شرح گروه ست شد به:\n'..about
end
local function get_description(msg, data)
  local data_cat = 'description'
  if not data[tostring(msg.to.id)][data_cat] then
    return 'شرحی برای گروه قرارداده نشده.'
  end
...
local function lock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'yes' then
    return 'محافظت ازگروه دربرابرربات هافعال است.'
  else
    data[tostring(target)]['settings']['lock_bots'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'محافظت ازگروه دربرابرربات هافعال شد.'
  end
end
...
local function unlock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'no' then
    return 'محافظت ازگروه دربرابرربات هاغیرفعال است.'
  else
    data[tostring(target)]['settings']['lock_bots'] = 'no'
    save_data(_config.moderation.data, data)
    return 'محافظت ازگروه دربرابرربات غیرفعال شد.'
  end
end
...
local function unlock_group_floodmod(msg, data, target)
  ...
  if not is_owner(msg) then
    return "تنها اونروادمین گروه میتواند حساسیت اسپم را باز کند."
  end
  local group_flood_lock = data[tostring(target)]['settings']['flood']
  if group_flood_lock == 'no' then
    return 'حساسیت اسپم قفل نیست.'
  else
    data[tostring(target)]['settings']['flood'] = 'no'
    save_data(_config.moderation.data, data)
    return 'حساسیت اسپم بازشد.'
  end
end
...
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'yes' then
    return 'اعضاگروه قفل است.'
  else
    data[tostring(target)]['settings']['lock_member'] = 'yes'
    save_data(_config.moderation.data, data)
  end
  return 'اعضاگروه قفل شد.'
end
...
local function unlock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'no' then
    return 'اعضاگروه قفل نیست.'
  else
    data[tostring(target)]['settings']['lock_member'] = 'no'
    save_data(_config.moderation.data, data)
    return 'اعضاگروه باز شد.'
  end
end
...
local function lock_group_leave(msg, data, target)
  ...
  if leave_ban == 'yes' then
    return 'لفت دادن کاربران برابربن شدن انهاست.'
  else
...
local function unlock_group_leave(msg, data, target)
  ...
  if leave_ban == 'no' then
    return 'لفت دادن کاربران برابربن شدن انها نیست.'
  else
...
local function lock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'yes' then
    return 'ارسال اسایکر قفل است.'
  else
    data[tostring(target)]['settings']['lock_sticker'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'ارسال استیکرقفل شد.'
  end
end
...
local function realmadd(msg)
  ...
  if is_realm(msg) then
    return 'گروه مدیران اضافه شده است.'
  end
    receiver = get_receiver(msg)
...
local function set_group_photo(msg, success, result)
  ...
  if success then
    ...
    send_large_msg(receiver, 'خطا,لطفا دوباره امتحان کنید!', ok_cb, false)
  end
end
...
local function promote(receiver, member_username, member_id)
  ...
  if data[group]['moderators'][tostring(member_id)] then
    return send_large_msg(receiver, member_username..' کمک مدیر است.')
  end
  data[group]['moderators'][tostring(member_id)] = member_username
  save_data(_config.moderation.data, data)
  return send_large_msg(receiver, member_username..' به عنوان کمک مدیر اضافه شد.')
end
...
local function demote(receiver, member_username, member_id)
  ...
  if not data[group]['moderators'][tostring(member_id)] then
    return send_large_msg(receiver, member_username..' کمک مدیر نمیباشد.')
  end
  data[group]['moderators'][tostring(member_id)] = nil
  save_data(_config.moderation.data, data)
  return send_large_msg(receiver, member_username..' ازکمک مدیری خلع شد.')
end
...
local function setowner_by_reply(extra, success, result)
  ...
  data[tostring(msg.to.id)]['set_owner'] = tostring(msg.from.id)
      save_data(_config.moderation.data, data)
      savelog(msg.to.id, name_log.." ["..msg.from.id.."] کاربر ["..msg.from.id.."] به عنوان صاحب گروه اضافه شد")
...
local function mute_user_callback(extra, success, result)
	...
	if is_muted_user(chat_id, user_id) then
		...
		send_large_msg(receiver, "["..user_id.."] ازلیست کاربران لال خارج شد.")
	else
  ...
  if is_admin1(msg) or not is_support(msg.from.id) then-- Admin only
	  if matches[1] == 'add' and not matches[2] then
	   ...
	   if matches[1] == 'add' and matches[2] == 'realm' then
		...
		if is_group(msg) then
		   return 'خطا,گروه میباشد.'
		end
		savelog(msg.to.id, name_log.." ["..msg.from.id.."] added realm [ "..msg.to.id.." ]")
		print("group "..msg.to.print_name.."("..msg.to.id..") اضافه شد به عنوان گروه مدیران")
		return realmadd(msg)
	  end
 ...
 --[[Experimental
  ...
  if msg.to.id and data[tostring(msg.to.id)] then
    ...
    if matches[1] == 'promote' and not matches[2] then
      ...
      if not is_owner(msg) then
        return "تنها صاحب گروه میتواند یک کمک مدیر اضافه کند."
      end
    ...
    if matches[1] == 'promote' and matches[2] then
      ...
      if not is_owner(msg) then
        return "فقط صاحب گروه میتواند یک کمک مدیر اضافه کند."
      end
    ...
    if matches[1] == 'demote' and not matches[2] then
      ...
      if not is_owner(msg) then
        return "فقط صاحب گروه میتواند یک کمک مدیر راخلع کنند."
      end
      if type(msg.reply_id)~="nil" then
    ...
    if matches[1] == 'demote' and matches[2] then
      ...
      if string.gsub(matches[2], "@", "") == msg.from.username and not is_owner(msg) then
        return "شما نمیتوانید خودتان را خلع کنید."
      end
	local member = matches[2]
  ...
  if matches[1] == 'mute' and is_owner(msg) then
			...
			local msg_type = 'Audio'
				if not is_muted(chat_id, msg_type..': yes') then
					...
					return "Group "..matches[2].." بی صدا شد."
				else
			...
			local msg_type = 'Photo'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set group to: mute "..msg_type)
					mute(chat_id, msg_type)
					return "گروه "..matches[2].." بی صدا شد."
				else
					return "گروه "..matches[2].."همچنان بی صداست."
				end
			end
			...
			local msg_type = 'Gifs'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set group to: mute "..msg_type)
					mute(chat_id, msg_type)
					return msg_type.." بی صدا شد"
				else
					return "گروه  "..msg_type.." بیصدا شد."
				end
			end
			...
			local msg_type = 'Text'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set group to: mute "..msg_type)
					mute(chat_id, msg_type)
					return "گروه text بی صدا شد."
				else
					return " گروه text همچنان بیصداست"
				end
			end
