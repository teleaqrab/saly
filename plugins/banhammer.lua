...
local function pre_process(msg)
  ...
  if msg.to.type == 'chat' or msg.to.type == 'channel' then
    ...
    if banned or is_gbanned(user_id) then -- Check it with redis
      ...
      local print_name = user_print_name(msg.from):gsub("‮", "")
	  local name = print_name:gsub("_", "")
      savelog(msg.to.id, name.." ["..msg.from.id.."] یوزردرحال صحبت بن شد !")-- Save to logs
...
local function kick_ban_res(extra, success, result)
	  ...
	  if success == 0 then
		return send_large_msg(receiver, "یوزرنیم مورد نظر پیدا نشد!")
	  end
      ...
      local get_cmd = extra.get_cmd
       if get_cmd == "kick" then
         if member_id == from_id then
            send_large_msg(receiver, "شمانمیتوانید خودتان را حذف کنید")
			return
         end
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
            send_large_msg(receiver, "شما نمیتوانید ادمین اصلی ربات و ادمین اصلی گروه راحذف کنید")
			return
         end
      ...
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] گلوبال ان بن شد.')
...
if matches[1]:lower() == 'kick' then
	...
	elseif string.match(matches[2], '^%d+$') then
		...
		if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
			return "شما نمیتوانید ادمین گروه و ادمین ربات راحذف کنید."
		end
		if tonumber(matches[2]) == tonumber(msg.from.id) then
			return "شما نمیتوانید خودتون رو کیک کنید!"
		end
    local user_id = matches[2]
    local chat_id = msg.to.id
		local print_name = user_print_name(msg.from):gsub("‮", "")
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return 'کاربر ['..user_id..' ] از گلوبال بن درامد.'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unbanall',
